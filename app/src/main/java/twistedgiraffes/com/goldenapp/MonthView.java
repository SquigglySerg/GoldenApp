...
public class MonthView extends TableLayout{
    ...
    private void checkForEvents() {
        ...
        Calendar event = (Calendar) cal.clone();
        event.set(Calendar.DATE, 1);
        event.clear(Calendar.HOUR);
        event.clear(Calendar.MINUTE);
        event.clear(Calendar.SECOND);
        Calendar tempcal1 = (Calendar) event.clone();
        tempcal1.set(Calendar.DATE, event.getActualMaximum(Calendar.DATE));
        ...
        for(int i = 1; i < 32; i++) {
            for (Event e : mDataBase.getEventList()) {
                ...
                Log.d("****Calen Month",months[cal.get(Calendar.MONTH)]);
                Log.d("****Event Month",date.substring(0,pos-2));

                //Log.d("****Calen Year",Calendar.YEAR);
                //Log.d("****Event Year",date.substring(pos+1, date.length()-1));
                if (pos > 0 && date.substring(pos - 1, pos).equals(Integer.toString(i)) && date.substring(0, pos-2).equals(months[cal.get(Calendar.MONTH)])
                        /*&& date.substring(pos+1, date.length()-1).equals(Calendar.YEAR) */){
                    isEvent[i] = true;
                    events[i] = e;
                }
            }
